    ...
    <div className="grid gap-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Draw Eligibility</span>
            <span className="text-2xl">🎯</span>
          </div>
          <div className={`text-2xl font-bold ${milestoneReached ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400'}`}>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border-2 border-slate-200 dark:border-slate-700 shadow-lg p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Add Draw Request</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Request project fund draws</p>
          </div>
        ...
        {showDrawForm && (
          <div className="space-y-5 bg-slate-50 dark:bg-slate-700 rounded-xl p-6">
            <div>
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Draw Amount ($) *</label>
              <input
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-700">
          {draws.length === 0 ? (
            <div className="p-6 text-sm text-slate-500 dark:text-slate-400 text-center">No draw requests yet</div>
          ) : (
            draws.map(d => (
              <div key={d.id} className="p-6 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex-1">
                    <h5 className="font-semibold text-slate-900 dark:text-white">Draw Request</h5>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">${d.amount?.toLocaleString() || 0}</p>
                  </div>
                ...
                {role === 'portfolio_manager' && (
                  <div className="flex gap-2">
                    <button
                      className="px-4 py-2 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-medium hover:bg-slate-100 dark:hover:bg-slate-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <button
                      className="px-4 py-2 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-medium hover:bg-slate-100 dark:hover:bg-slate-700 transition-all"
